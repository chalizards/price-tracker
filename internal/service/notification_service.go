...
func (service *NotificationService) CheckPriceNotifications(ctx context.Context, product *models.Product, offer *models.Offer, newPrice *models.Price) {
	service.checkTargetReached(ctx, product, offer, newPrice)
	service.checkPriceDrop(ctx, product, offer, newPrice)
}
...
func (service *NotificationService) checkTargetReached(ctx context.Context, product *models.Product, offer *models.Offer, newPrice *models.Price) {
	if product.TargetPrice == nil || newPrice.Price > *product.TargetPrice {
		return
	}

	title := fmt.Sprintf("Target price reached for %s at %s", product.Name, offer.Name)
...
func (service *NotificationService) checkPriceDrop(ctx context.Context, product *models.Product, offer *models.Offer, newPrice *models.Price) {
	previousPrices, err := service.priceRepo.GetByOfferID(ctx, offer.ID, newPrice.PaymentType)
	...
	title := fmt.Sprintf("Price drop for %s at %s (%s)", product.Name, offer.Name, newPrice.PaymentType)
