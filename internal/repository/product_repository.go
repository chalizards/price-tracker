...
func (repo *ProductRepository) Create(ctx context.Context, product *models.Product) error {
	query := `
		INSERT INTO products (name, target_price, active)
		VALUES ($1, $2, $3)
		RETURNING id, name, target_price, active, created_at, updated_at
	`
	return repo.db.QueryRow(ctx, query,
		product.Name, product.TargetPrice, product.Active,
	).Scan(&product.ID, &product.Name, &product.TargetPrice, &product.Active, &product.CreatedAt, &product.UpdatedAt)
}

func (repo *ProductRepository) GetByID(ctx context.Context, id int) (*models.Product, error) {
	query := `
		SELECT id, name, target_price, active, created_at, updated_at
		FROM products
		WHERE id = $1
	`
	product := &models.Product{}
	err := repo.db.QueryRow(ctx, query, id).Scan(
		&product.ID, &product.Name, &product.TargetPrice,
...
func (repo *ProductRepository) Update(ctx context.Context, product *models.Product) (*models.Product, error) {
	...
	err := repo.db.QueryRow(ctx, query,
		...
		&updatedProduct.ID, &updatedProduct.Name, &updatedProduct.TargetPrice,
