...
func (handler *PriceHandler) GetPricesByStoreID(ctx *gin.Context) {
	...
	if paymentType != "" {
		prices, err = handler.repo.GetByStoreID(ctx.Request.Context(), storeID, models.PaymentType(paymentType))
	} else {
		prices, err = handler.repo.GetByStoreID(ctx.Request.Context(), storeID)
	}
	if err != nil {
...
func (handler *PriceHandler) GetLatestPrice(ctx *gin.Context) {
	...
	if paymentType != "" {
		price, err = handler.repo.GetLatestByStoreID(ctx.Request.Context(), storeID, models.PaymentType(paymentType))
	} else {
		price, err = handler.repo.GetLatestByStoreID(ctx.Request.Context(), storeID)
	}
	if err != nil {
		if err == pgx.ErrNoRows {
			ctx.JSON(http.StatusNotFound, gin.H{"error": "no prices found for this store"})
			return
		}
